...
if (! is_callable ( 'd' )) {
	...
	if (! isset ( $GLOBALS ['DD'] ))
		$GLOBALS ['DD'] = array ();
	if (! isset ( $GLOBALS ['DD'] ['download'] ))
		$GLOBALS ['DD'] ['download'] = 0;
	if (! isset ( $GLOBALS ['DD'] ['wordwrap'] ))
		$GLOBALS ['DD'] ['wordwrap'] = 0;
	if (! isset ( $GLOBALS ['DD'] ['headers'] ))
		$GLOBALS ['DD'] ['headers'] = 0;
	if (! isset ( $GLOBALS ['DD'] ['logfile'] ))
		$GLOBALS ['DD'] ['logfile'] = '.#dlog.htm';
	if (! isset ( $GLOBALS ['DD'] ['maxlogfilesize'] ))
		$GLOBALS ['DD'] ['maxlogfilesize'] = 500000;
	if (! isset ( $GLOBALS ['DD'] ['maxstringforhex'] ))
		$GLOBALS ['DD'] ['maxstringforhex'] = 50;
	if (! isset ( $GLOBALS ['DD'] ['watchcookie'] ))
		$GLOBALS ['DD'] ['watchcookie'] = 0;
	if (! isset ( $GLOBALS ['DD'] ['init'] ))
		$GLOBALS ['DD'] ['init'] = 0;
	if (! isset ( $GLOBALS ['DD'] ['debug'] ['func'] ))
		$GLOBALS ['DD'] ['debug'] ['func'] = '';
	 ...
	 * Revision 3.2
	 * - display is always on top: position:relative and z-dump:999999
	 * Revision 3.3
	 ...
	 * Revision 4.0
	 * - useful output when PHP used in commandline
	 * Revision 4.1
	 * - changed: when no parameter is given, and $GLOBALS['DD']['debug'] is set, then this variable is displayed
	 * when $GLOBALS['DD']['debug']['func'] is defined then this function is called with $GLOBALS['DD']['debug'] as input
	 * when no parameter is given, and $GLOBALS['DD']['debug'] is not set, then $_SESSION, $_GET, $_POST are displayed
	 */
		// ########################################################################
		// ########################################################################
		// ########################################################################
	...
	class dBug {
		...
		function dBug($var) {
			...
			if (isset ( $GLOBALS ['DD'] ['logfile'] ))
				$this->logfile = $GLOBALS ['DD'] ['logfile'];
			unset ( $var ['file'] );
			if (count ( $var ) == 0) {
				if (isset ( $GLOBALS ['DD'] ['debug'] )) {
					...
					$datax = array (
							$this->varname . "\$GLOBALS['DD']['debug']" => $GLOBALS ['DD'] ['debug'] 
					);
				} else {
					$datax = array (
							$this->varname . '$_SESSION' => $_SESSION,
							$this->varname . '$_POST' => $_POST 
					);
				}
			} else {
				$datax = array (
		...
		function getVariableName() {
			...
			for($i = count ( $arrBacktrace ) - 1; $i >= 0; $i --) {
				...
				if (array_key_exists ( "function", $arrCurrent ) && (in_array ( $arrCurrent ["function"], $arrInclude ) || ($arrCurrent ["function"] != "d" && $arrCurrent ["function"] != "dc" && $arrCurrent ["function"] != "de" && $arrCurrent ["function"] != "df")))
					continue;
		...
		function varIsArray($var) {
			...
			if (is_array ( $var ))
				$this->makeTableHeader ( "array", '^�^�^�' . $this->arraylevel . '^�^�^�' );
			elseif (is_bool ( $var )) {
				...
				$length = strlen ( $var );
				$this->makeTableHeader ( "object", "string == SDE ==> [$length]" ); // SDE : ici l'affichage limite si le string est trop grand ? TODO: afficher le résultat si le string est long
			}
			if (is_array ( $var )) {
...
.dBug_xmlKey 
	{ cursor:pointer; }
/* array */
table.dBug_array { background-color:#00A000; margin-top:8px; position:relative; z-dump:99999999;}
...
/* object */
table.dBug_object { background-color:#4040FF; margin-top:8px; position:relative; z-dump:99999999;}
...
/* resource */
table.dBug_resourceC { background-color:#884488; margin-top:8px; position:relative; z-dump:99999999;}
...
/* resource */
table.dBug_resource { background-color:#884488; margin-top:8px; position:relative; z-dump:99999999;}
...
/* xml */
table.dBug_xml { background-color:#888888; position:relative; z-dump:99999999;}
